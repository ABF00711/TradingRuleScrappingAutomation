"""
Test script for Lucid Trading extractor
"""
...
from propfirm_scraper.extractors.lucidtrading import LucidTradingExtractor
from propfirm_scraper.core.logger import setup_logger
from propfirm_scraper.config.schema import SiteConfig

logger = setup_logger()

async def test_lucidtrading_extractor():
    """Test the Lucid Trading extractor"""
    
    print("=" * 60)
    print("TESTING LUCID TRADING EXTRACTOR")
    print("=" * 60)
    ...
    try:
        ...
        # Create site config (using support center as configured)
        print("2. Creating site configuration...")
        site_config = SiteConfig(
            name="Lucid Trading",
            url="https://support.lucidtrading.com/en/",
            extractor_class="LucidExtractor"
        )
        
        # Initialize extractor
        print("3. Initializing Lucid Trading extractor...")
        extractor = LucidTradingExtractor(site_config)
        ...
        print("\n" + "=" * 60)
        print("LUCID TRADING EXTRACTOR TEST COMPLETED")
        print("=" * 60)
...
if __name__ == "__main__":
    asyncio.run(test_lucidtrading_extractor())