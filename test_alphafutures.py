"""
Test script for Alpha Futures extractor
"""
...
from propfirm_scraper.extractors.alphafutures import AlphaFuturesExtractor
from propfirm_scraper.core.logger import setup_logger
from propfirm_scraper.config.schema import SiteConfig

logger = setup_logger()

async def test_alphafutures_extractor():
    """Test the Alpha Futures extractor"""
    
    print("=" * 60)
    print("TESTING ALPHA FUTURES EXTRACTOR")
    print("=" * 60)
    ...
    try:
        ...
        site_config = SiteConfig(
            ...
            extractor_class="AlphaFuturesExtractor"
        )
        
        # Initialize extractor
        print("3. Initializing Alpha Futures extractor...")
        extractor = AlphaFuturesExtractor(site_config)
        ...
        try:
            ...
            for i, rule in enumerate(trading_rules):
                ...
                print(f"     Daily Loss Limit: ${rule.evaluation_daily_loss_usd:,.0f}" if rule.evaluation_daily_loss_usd else "     Daily Loss Limit: N/A")
                print(f"     Drawdown Type: {rule.evaluation_drawdown_type}" if rule.evaluation_drawdown_type else "     Drawdown Type: N/A")
                print(f"     Min Trading Days: {rule.evaluation_min_days}" if rule.evaluation_min_days else "     Min Trading Days: N/A")
        ...
        print("\n" + "=" * 60)
        print("ALPHA FUTURES EXTRACTOR TEST COMPLETED")
        print("=" * 60)
...
if __name__ == "__main__":
    asyncio.run(test_alphafutures_extractor())